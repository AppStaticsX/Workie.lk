...
class _PostTabPageState extends State<PostTabPage> {
  ...
  final List<String> _jobUrgencyLevels = [
  ...
  /*Widget _buildWorkersNeededSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
        ...
        TextFormField(
          ...
          onChanged: (value) {
            if (_isWorkersNeededEmpty && value.isNotEmpty) {
              setState(() => _isWorkersNeededEmpty = false);
            }
          },
          decoration: InputDecoration(
            hintText: 'E.g. 2 Masons + 1 Helper',
            hintStyle: const TextStyle(color: Colors.grey),
            filled: true,
            fillColor: Theme.of(context).colorScheme.surface,
            contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
            ...
            enabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
              borderSide: BorderSide(
                color: _isWorkersNeededEmpty ? Colors.red : Theme.of(context).colorScheme.outline.withValues(alpha: 0.5),
                width: 1.5,
              ),
            ),
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
              borderSide: BorderSide(
                color: _isWorkersNeededEmpty ? Colors.red : const Color(0xFF4E6BF5),
                width: 2.0,
              ),
            ),
          ),
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          items: _jobUrgencyLevels.map((urgency) {
            return DropdownMenuItem(
              value: urgency,
              child: Text(urgency),
            );
          ...
          onChanged: (value) {
            setState(() => _selectedJobUrgency = value);
          },
        ),
      ],
    );
  ...
  bool _validateForm() {
    ...
    if (_isJobTitleEmpty || _isJobCategoryEmpty || _isJobDescriptionEmpty ||
        _isLocationEmpty || _isWorkersNeededEmpty || _isContactEmpty || _isBudgetEmpty) {
      isValid = false;
    }
