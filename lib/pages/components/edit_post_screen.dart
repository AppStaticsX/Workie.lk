...
class EditPostScreen extends StatefulWidget {
  ...
  final Map<String, dynamic>? postToEdit; // Add this parameter for editing

  const EditPostScreen({
    super.key,
    this.onPostSuccess,
    this.postToEdit, // Add this parameter
  });
...
class _EditPostScreenState extends State<EditPostScreen> {
  ...
  // Add property to track if we're editing
  ...
  void _initializeEditData() {
    if (_isEditing && widget.postToEdit != null) {
      ...
      if (post['hashtags'] != null) {
        selectedHashtags = List<String>.from(post['hashtags']);
      }
      
      // Set location
      _pickedLocationAdress = post['location'] ?? '';
      
      // Note: Media cannot be edited, so we don't populate _selectedImages or _selectedVideos
    }
  }
  ...
  Widget _buildExistingMediaPreview() {
    final mediaUrls = widget.postToEdit!['mediaUrls'] as List;
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Text(
                  'Existing Media (Cannot be modified)',
                  style: TextStyle(
                    fontSize: 12,
                    color: Colors.grey[600],
          ...
          SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final isVideo = media.type.toString().contains('video');
                ...
                return Container(
                  ...
                  child: Stack(
                    children: [
                      ...
                      // Lock icon to indicate disabled state
                      Positioned(
                        top: 4,
                        right: 4,
                        child: Container(
  ...
  Future<void> _handleUploadPost() async {
    ...
    try {
      if (_isEditing) {
        // Handle editing existing post (content and hashtags only)
        await _handleUpdatePost(token);
      } else {
        // Handle creating new post (existing logic)
        await _handleCreatePost(token);
      }
    } catch (e) {
      // Show error notification
      await NotificationService.showNotification(
        title: _isEditing ? 'Update Failed' : 'Upload Failed',
        body: 'Failed to ${_isEditing ? 'update' : 'create'} post: ${e.toString()}',
        payload: _isEditing ? 'update_error' : 'upload_error',
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to ${_isEditing ? 'update' : 'create'} post: $e')),
        );
      }
    }
  }

  Future<void> _handleUpdatePost(String token) async {
    final postId = widget.postToEdit!['id'];
    
    // Show progress notification
    await _showProgressNotification(50, 100, 'Updating post...');

    // Update post using PostDataService
    final result = await PostDataService.updatePost(
      postId: postId,
      content: _textController.text,
      hashtags: selectedHashtags,
      location: _pickedLocationAdress.isNotEmpty ? _pickedLocationAdress : null,
    );

    // Show final progress
    await _showProgressNotification(100, 100, 'Post updated successfully!');
  ...
  Future<void> _handleCreatePost(String token) async {
    ...
    return Scaffold(
                ...
                onPressed: (){
                  showDialog(
                    ...
                    builder: (context) => AIContentWriterDialog(
                      selectedHashtags: [], // Pass empty list so no hashtags are shown in the dialog
                      onContentGenerated: (String content) {
                        setState(() {
                          _textController.text = content;
                          selectedHashtags.clear(); // Clear existing hashtags when AI generates new content
                        });
                      },
                    ),
                  );
      ...
      body: Column(
        children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'You can only edit text content and hashtags. Media files cannot be modified.',
                      style: TextStyle(
                        fontSize: 12,
          ...
          // Display existing media when editing (disabled/read-only)
          if (_isEditing && widget.postToEdit != null && widget.postToEdit!['mediaUrls'] != null) ...[
            _buildExistingMediaPreview(),
          ],
          // Display selected images (only for new posts, not when editing)
          if (_selectedImages.isNotEmpty && !_isEditing) ...[
            Container(
              margin: const EdgeInsets.only(left: 16, right: 16, top: 16, bottom: 16),
              height: 120,
              child: ListView.builder(
          ...
          // Display selected videos (only for new posts, not when editing)
          if (_selectedVideos.isNotEmpty && !_isEditing) ...[
            Container(
              margin: const EdgeInsets.only(left: 16, right: 16, top: 16, bottom: 16),
              height: 120,
              child: ListView.builder(
                    ...
                    children: [
                      IconButton(
                        icon: Icon(
                          ...
                          color: _isEditing ? Colors.grey[600] : Colors.grey,
                          size: 28,
                        ),
